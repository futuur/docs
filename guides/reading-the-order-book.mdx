...
Instead of polling `GET /markets/{id}/book/`, subscribe to the per-event Pusher channel `event-{event_id}` and listen for `order-book-update`. The event carries a refetch ping — call the REST [order book](/api-reference/markets/order-book) endpoint when you receive it. See [Channels and events](/api-reference/websocket/channels-and-events) for setup and payload details.

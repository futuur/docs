...
<Note>
  On-chain CLOB trading is coming soon. Matching will stay off-chain (user orders first, including complementary both-buy). Collateral will move on-chain at match, and each create will require an EIP-712 wallet signature in addition to HMAC. See [On-chain trading](/concepts/on-chain-trading).
</Note>

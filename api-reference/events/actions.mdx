---
title: 'Event actions'
description: 'List all trading actions for a specific event.'
---

Retrieve a list of all trading actions (wagers) for an event. Real money actions have user information anonymized.
...
<ParamField query="aggregate_by_orders" type="boolean" default="true">
  When `true`, aggregate actions by orders.
</ParamField>

<ParamField query="my_bets" type="boolean" default="false">
  When `true`, return only your own bets.
</ParamField>

<ParamField query="following" type="boolean" default="false">
  When `true`, return only bets from users you follow.
</ParamField>

...
Every authenticated request requires HMAC-SHA512 authentication using a public/private key pair. See the [Authentication](/authentication) page for the full signing process.

<Note>
  Some endpoints (like listing events) do not require authentication. Check each endpoint's documentation for requirements.
</Note>

## For AI agents

The full OpenAPI schema is available at `https://api.futuur.com/docs/schema/`. This endpoint provides the complete API specification in JSON format, perfect for code generation, API clients, and AI-powered integrations.
